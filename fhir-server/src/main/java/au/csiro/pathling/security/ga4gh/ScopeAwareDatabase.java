...
public class ScopeAwareDatabase extends Database {
  ...
  public ScopeAwareDatabase(@Nonnull final ServerConfiguration configuration,
    ...
    super(configuration.toDatabaseConfiguration(), spark, fhirEncoders, executor);
  ...
  public Dataset<Row> read(@Nonnull final ResourceType resourceType) {
    ...
    return passportScope
        .map(scope -> {
          ...
          final Database database = new Database(configuration.toDatabaseConfiguration(), spark,
              fhirEncoders, executor);
