...
class PathlingContext:
  ...
  def create(
    ...
    storage_config = (
      jvm.au.csiro.pathling.config.StorageConfiguration.builder()
        .warehouseUrl(warehouse_url)
        .build()
    ) if warehouse_url is not None else None

    jpc: JavaObject = jvm.au.csiro.pathling.library.PathlingContext.create(
      spark._jsparkSession, encoders_config, terminology_config, storage_config
    )
